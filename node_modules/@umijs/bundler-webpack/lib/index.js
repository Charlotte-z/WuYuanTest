...
class Bundler {
  ...
  setupDevServerOpts({
    ...
    const _this$config = this.config,
          devServer = _this$config.devServer,
          outputPath = _this$config.outputPath; // 这里不做 winPath 处理，是为了和下方的 path.sep 匹配上

    const absOutputPath = (0, _path().join)(this.cwd, outputPath); // @ts-ignore
